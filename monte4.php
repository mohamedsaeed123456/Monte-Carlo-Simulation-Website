...
echo "</table>";
 echo '<h1>The Expected
daily
demand is </h1>';
 echo(number_format($row['b'] / $total,2)*$row['a'] + number_format($row['s'] / $total,2) *  $row['c'] + number_format($row['f'] / $total,2) * $row['d'] +  number_format($row['h'] / $total,2) * $row['g'] + number_format($row['k'] / $total,2) * $row['j'] + number_format($row['n'] / $total,2) * $row['m']);
 echo "  tires";
 }
